...
GaussianProcess::GaussianProcess (size_t input_dim, size_t info_dim, std::string covf_def):
  info_dim_(info_dim)
{
    // set input dimensionality
    this->input_dim = input_dim;
    ...
    cf = factory.create(input_dim, covf_def);
    cf->loghyper_changed = 0;
    sampleset = new SampleSet(input_dim, info_dim_);
    L.resize(initial_L_size, initial_L_size);
}
